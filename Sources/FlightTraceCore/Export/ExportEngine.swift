...
public final class ExportEngine: Sendable {
    ...
    /// - CGContext: The graphics context to render into
    ...
    public func setRenderingClosure(
        _ closure: @escaping @Sendable (CGContext, CGSize, Date, Int) -> Void
    ) {
    ...
    public func export(
        ...
        for frameNumber in 0..<totalFrames {
            ...
            // Render this frame
            do {
                try renderer.renderFrame(at: presentationTime) { context, size in
                    renderingClosure(context, size, timestamp, frameNumber)
                }
            } catch {
...
private final class RenderingClosureBox: @unchecked Sendable {
    private var closure: (@Sendable (CGContext, CGSize, Date, Int) -> Void)?
    private let lock = NSLock()

    func setClosure(_ newClosure: @escaping @Sendable (CGContext, CGSize, Date, Int) -> Void) {
        lock.lock()
        defer { lock.unlock() }
        closure = newClosure
    }

    func getClosure() -> (@Sendable (CGContext, CGSize, Date, Int) -> Void)? {
        lock.lock()
        defer { lock.unlock() }
        return closure
    }
