...
extension UTType {
    ...
    static var gpx: UTType {
        // Use a tag-based lookup so we don't require an Info.plist declaration
        if let type = UTType(filenameExtension: "gpx", conformingTo: .xml) {
            return type
        }
        // Fallback to XML if the specific type can't be resolved
        return .xml
    }
}
